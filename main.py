...
# ================= ইম্পোর্ট ব্যাকআপ ম্যানেজার =================
import backup_manager as bm

# ব্যাকআপ ম্যানেজারে ডাটা রেফারেন্স সেট করুন
bm.set_data_refs(
    user_active_ticket, ticket_status, ticket_user, ticket_username,
    ticket_messages, user_tickets, ticket_created_at, user_latest_username
)
...
async def backup_command(update: Update, context):
    ...
    zip_path, btype, ts = bm.create_backup("manual")
    if zip_path:
        caption = (
            f"🔐 **Manual Backup**\n"
            f"🕒 Time: {get_bst_now()}\n"
...
async def restore_command(update: Update, context):
    ...
    if not update.message.reply_to_message or not update.message.reply_to_message.document:
        await update.message.reply_text("❌ Reply to a backup ZIP file with /restore")
        return
...
async def password_callback(update: Update, context):
    ...
    await query.answer()
    await query.edit_message_text("🔑 Use: /password Blockveil123*#%", parse_mode="Markdown")

async def password_command(update: Update, context):
    if update.effective_chat.id != BACKUP_GROUP_ID:
        return
    if not context.args:
        await update.message.reply_text("❌ Give password! Example: /password Blockveil123*#%")
        return
    password = context.args[0]
    file_id = context.user_data.get('restore_file_id')
    if not file_id:
        await update.message.reply_text("❌ Use /restore first.")
        return
    status_msg = await update.message.reply_text("🔄 Restoring...")
    try:
        file = await context.bot.get_file(file_id)
        temp_path = os.path.join(bm.BACKUP_DIR, f"temp_{datetime.now().strftime('%Y%m%d%H%M%S')}.zip")
        await file.download_to_drive(temp_path)
        success, msg = bm.restore_from_backup(temp_path, password)
        os.remove(temp_path)
...
async def unknown_backup_command(update: Update, context):
    if update.effective_chat.id == BACKUP_GROUP_ID:
        await update.message.reply_text("❌ Only /backup, /restore, /password work here.")
...
async def send_photo(update: Update, context): await send_media(update, context, "photo")
async def send_document(update: Update, context): await send_media(update, context, "document")
async def send_audio(update: Update, context): await send_media(update, context, "audio")
async def send_voice(update: Update, context): await send_media(update, context, "voice")
async def send_video(update: Update, context): await send_media(update, context, "video")
async def send_animation(update: Update, context): await send_media(update, context, "animation")
async def send_sticker(update: Update, context): await send_media(update, context, "sticker")

# ================= INIT =================
app = ApplicationBuilder().token(TOKEN).build()

# Backup group handlers (must be added first)
...
backup_thread = threading.Thread(target=bm.auto_backup_loop, args=(app, BACKUP_GROUP_ID), daemon=True)
backup_thread.start()

print("🤖 Bot started successfully!")
print(f"📊 Support Group: {GROUP_ID}")
print(f"📦 Backup Group: {BACKUP_GROUP_ID}")
print(f"🔑 Backup Password: {bm.BACKUP_PASSWORD}")
